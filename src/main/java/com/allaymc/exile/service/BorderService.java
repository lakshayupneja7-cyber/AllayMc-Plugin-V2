...
public class BorderService {
    ...
    public void showMainBorderHit(Player player, Location attempted) {
        showBarrierFeedback(player, attempted, true);
    }

    public void showExileBorderHit(Player player, Location attempted) {
        showBarrierFeedback(player, attempted, false);
    }
    ...
    public void applyKnockback(Player player, Location from, Location to) {
        ...
        Vector push = from.toVector().subtract(to.toVector()).normalize()
                .multiply(plugin.getConfig().getDouble("danger-zone.pushback-strength", 1.55));
        push.setY(0.3);
        player.setVelocity(push);
    }

    public void showBarrierFeedback(Player player, Location attempted, boolean main) {
        String message = main
                ? plugin.getConfig().getString("barrier.main-message", "&cA mysterious force blocks you at the server border.")
                : (player.hasPermission("allaymc.exile.self") && player.isOnline()
